  ...
  if cleanTile then
    ...
    if not targetTile:isWalkable() then
      ...
      if topThing == 2129 or topThing == 2130 or topThing == customMwall then
        if targetTile:getTimer() < pushDelay+300 then
          vBot.isUsing = true
          schedule(pushDelay+300, function()
            vBot.isUsing = false
          end)
        end
    ...
    if not tileOfTarget:getTopUseThing():isNotMoveable() and targetTile:getTimer() < pushDelay+300 then
